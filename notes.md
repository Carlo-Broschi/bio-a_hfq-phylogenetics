...
**重要な注意（bio-b と事情が違う）：** 構造ガイド版は素の版（225×**254**列）より**大幅に短い**。bio-a は同じ細菌 Hfq 内で変異領域も相同＝系統情報を持ちうるため、構造ガイドの絞り込みが**浅い枝の解像度を下げる懸念**がある（bio-b は3ドメインで生配列が並ばず構造が必須だったのと対照的）。→ 理論でなく実測で判断するため IQ-TREE3 で構造ガイド木を作り比較。

**比較結果（2026-07-04、確定）：**

| 版 | 列数 | モデル | UFBoot≥95 |
|---|---|---|---|
| 素の版（derogued ML） | 254 | Q.INSECT+G4 | **91 / 221（41%）** |
| 構造ガイド版 | 101 | Q.INSECT+I+G4 | 84 / 222（38%） |
...
### 2026-07-05 — 投稿直前フェーズ：GBE パッケージ・必須セクション・参考文献

- **構造ガイド MSA の効果（確定）**：構造ガイド版（225×101）は素の版（225×254）より UFBoot≥95 が **84 vs 91** で改善せず。bio-a は同 family で変異領域も相同=情報を持つため絞り込みが浅い枝を落とす、という予測どおり。→ 系統樹は主樹形（素の版 ML）を採用、構造ガイドは方法論一貫性の補助に留める。
- **Hfq 喪失一次文献**：Sun, Zhulin & Wartell 2002（NAR 30:3662、10.1093/nar/gkf508）を §3.4/Discussion に追加。Hfq は細菌の約半数のみ・遺伝子喪失が分布を規定・縮小ゲノム寄生体で欠如・Aquifex 保持。本文の「門追従＋vertical inheritance＋Aquificota 保有」主張の一次根拠。
- **必須セクション追加**：Significance statement（96語）＋ Data Availability を draft に追加（GBE Article 要件）。
- **GBE 投稿パッケージ**（`Biology/_gbe_submission/bio-a/`）：submission.md・参考文献リスト（11件、DOI 全検証・GBE 著者年式）・CMYK 300dpi TIFF（Fig.1 concordance / Fig.2 taxonomy）・compliance（全上限クリア：abstract 193w/title 59字/本文 1721w/refs 11/display items 3）。
- **参考文献**：`REFS_zotero_doi.txt`（11件、全 DOI CrossRef 検証）で Zotero 一括投入可。固定点：IQ-TREE3=Wong 2026（Minh2020=IQ-TREE2 は不使用）。
...
### 2026-07-05 — Hfq 喪失センサス（統合版 bio-c §3.3 の核）＋ bio-c 統合

- **[完了] Hfq 喪失センサス**：`fetch_bact_proteomes.py`（**Complete Genome 選抜**で内共生菌の CheckM 過小評価を回避）＋`hfq_census.py`（PF17209 --cut_ga）。**71 complete ゲノム・37 taxon・8生活様式**。3-analysis/hfq_census/。
  - **発見＝Hfq 喪失の2モード**：(A) 縮小随伴（内共生18%/Mollicute 0%/**偏性細胞内 0%**、Hfq+ 中央4.1Mb vs Hfq− 1.2Mb、点双列 r=0.34、Actinobacteria 除外 r=0.51、taxon 中央値でも同値＝擬似反復頑健）(B) **サイズ非依存の系統特異喪失**（Actinobacteria 0/8 が3-9Mb、Myxococcus 9Mb）。Aquificota 100%＝Sun 2002 確証。
  - **生活様式レビュー（プロ視点）**：`lifestyle_justification.md`。**Francisella を obligate_intracellular→facultative_host に再分類**（通性・axenic 培養可・Hfq 保持）→ 偏性細胞内が 0/10 に鮮明化。Actinobacteria は「系統特異的喪失のテスト群」と明示。
  - **caveat**：absence は cut_ga 非検出（Actinobacteria の canonical Hfq 欠如は文献既知、Myxococcus も文献で hfq 欠如確認済）。
- **[完了] bio-c 統合**：bio-a+bio-b を統合1本（salami 回避）。`Biology/bio-c_sm-fold-genome-reduction`（Zenodo 10.5281/zenodo.21202213）。bio-a は archived component。
- **[完了] 総点検（COPE accountability）**：全数値を実ファイルに照合（`bio-c/REVIEW_selfcheck.md`）。修正：TM ≥0.92→≥0.91（実 min 0.918）、型テーブル・トレーサビリティ、Hfq "peripheral" 過剰断定。**AI 使用開示文**を bio-c に追加。
